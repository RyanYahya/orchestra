...
4. **Install slash command adapters for each detected tool:**
   ...
   | Codex CLI | `adapters/codex/prompts/` | `.codex/prompts/` |
...
7. **Verify install:**
   - `ls .orchestra/prompts/` — should list 8 files (agent, archive, docs-sync, execute, execute-headless, plan, plan-advanced, resolve).
   - `ls .orchestra/scripts/` — should show `phase-runner.sh` and `orchestra/`.
   - For each detected tool, list the installed slash command directory and confirm the 8 wrappers are there.

8. **Report back to the user:**
   - Which tools you installed adapters for.
   - The slash command they can run next: `/orchestra:plan <task>` (Claude Code) or `/plan <task>` (others).
...
- Slash commands across every tool delegate to the same prompt bodies in `.orchestra/prompts/`. Edit the prompt file once and every tool's command updates.
