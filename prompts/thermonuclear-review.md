...
- **Cursor (2.4+):** use the **Task tool** to launch one subagent per lane **in parallel, in a single message** — multiple Task calls in one message run simultaneously. Name each lane explicitly. Pin a stronger model to the reasoning lanes and a cheaper one to the doc-checking specialists via each subagent's `model` field, and set `readonly: true`. Cursor natively discovers subagents in `.cursor/agents/`, `.claude/agents/`, and `.codex/agents/` but **not** `.orchestra/agents/`, so read each matching specialist file and pass its instructions inline. Cursor has a real subagent mechanism — do not treat it as lacking one.
