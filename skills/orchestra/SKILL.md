...
- During `execute`, `execute all`, and `execute-headless`, spawned-agent audit is mandatory after every phase. Treat `$orchestra execute`, `$orchestra execute all`, `/orchestra:execute`, and phase-runner execution as explicit user authorization to spawn subagents for the audit. In Codex, use `multi_agent_v1.spawn_agent` after tool discovery if needed; in Cursor (2.4+), use the Task tool to launch the audit subagents in parallel in a single message. Do not replace the spawned-agent audit with self-review when a subagent mechanism exists. In the autonomous runner (`execute all`, `execute-headless`/phase-runner), a residual audit issue does NOT halt the run — it is fixed best-effort and otherwise deferred to `Deferred_Issues.md`; only a verify build that stays red after its fix budget, or a genuine emergency, halts. If the host genuinely has no subagent mechanism, run a thorough self-review and note it in `Deferred_Issues.md` rather than blocking.
- In the Codex app, `$orchestra execute all` is also explicit authorization to create one fresh top-level Codex task for each next phase. Use the exact phase-handoff protocol in `references/execute-all.md`; do not fork the current task or use an audit subagent as the next-phase owner.
...
- This skill is the primary Codex surface. Prefer `$orchestra <command>` or natural language like "orchestra execute P2". Use `$orchestra execute` for the normal single-phase flow and `$orchestra execute all` for Codex-only autonomous execution through every remaining phase. In the Codex app, each completed phase hands the run to a fresh top-level task when the same local project can be preserved. For cleanup-only review, use `$simplify` directly or `$orchestra simplify`.
